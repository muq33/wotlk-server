...
function Majordomo.BlastWave(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature, Spells.BLAST_WAVE, true, damage_calc(4100))
end
...
function Majordomo.LivingBomb(eventId, delay, calls, creature)
    ...
    for i = 1, #sample_targets do
        creature:CastSpell(all_targets[sample_targets[i]], Spells
            .LIVING_BOMB, true)
    end
end
...
function Majordomo.OnEnterCombat(event, creature, target)
    creature:CastCustomSpell(creature, Spells.AEGIS_OF_RAGNAROS, true,
        creature:GetMaxHealth() / 20, 350)
end
...
function Flamewaker_Elite.ShadowShock(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature:GetVictim(),
        Flamewaker_Elite_Spells.SHADOW_SHOCK, true, damage_calc(1900))
end

function Flamewaker_Elite.FireBlast(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature:GetVictim(),
        Flamewaker_Elite_Spells.FIRE_BLAST, true, damage_calc(2450))
end
...
function Flamewaker_Elite.FireShadow(eventId, delay, calls, creature)
    ...
    if (which_to_cast == 1) then
        creature:CastCustomSpell(target, Flamewaker_Elite_Spells.SHADOW_BOLT,
            true, damage_calc(2650))
    else
        creature:CastCustomSpell(target, Flamewaker_Elite_Spells.FIREBALL, true,
            damage_calc(2650))
    end
end
...
function Flamewaker_Elite.SeparationAnxiety(event, creature, attacker, damage)
    ...
    if Majordomo == nil then
        creature:CastSpell(creature,
            Flamewaker_Elite_Spells.SEPARATION_ANXIETY_MINION, true)
    else
