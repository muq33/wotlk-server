...
function Gehennas.GehennasCurse(eventId, delay, calls, creature)
    ...
    for i = 1, #sample_targets do
        creature:CastSpell(all_targets[sample_targets[i]], Spells.GEHENNAS_CURSE,
            true)
    end
end
...
function Gehennas.RainfOfFire(eventId, delay, calls, creature)
    local all_targets = creature:GetAITargets()
    local sample_targets = sample(1, #all_targets, calculate_targets(2, #all_targets))
    for i = 1, #sample_targets do
        creature:CastCustomSpell(all_targets[sample_targets[i]],
            Spells.RAIN_OF_FIRE, true, damage_calc(4000))
    end
    
end

function Gehennas.ShadowBolt(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature:GetVictim(), Spells.SHADOW_BOLT, true, damage_calc(3800))
end
...
function Flamewalker.FistOfRagnaros(eventId, delay, calls, creature)
    creature:CastSpell(creature:GetVictim(), Flamewalker_Spells.FIST_OF_RAGNAROS,
        true)
end
