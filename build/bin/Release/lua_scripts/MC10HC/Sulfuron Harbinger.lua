...
function SulfuronHarbinger.Knockdown(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature, SulfuronHarbinger_Spells.KNOCKDOWN, true,
        nil, nil, damage_calc(5100))
end

function SulfuronHarbinger.DemoralizingShout(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature,
        SulfuronHarbinger_Spells.DEMORALIZING_SHOUT, true, -1001, -1001)
end
...
function SulfuronHarbinger.Flamespear(eventId, delay, calls, creature)
    ...
    for i = 1, #sample_targets do
        creature:CastCustomSpell(all_targets[1],
            SulfuronHarbinger_Spells.FLAMESPEAR, true, damage_calc(6100))
    end
...
function SulfuronHarbinger.OnEnterCombat(event, creature, target)
    ...
    creature:RegisterEvent(SulfuronHarbinger.DemoralizingShout, { 12000, 18000 },
        0)
...
function FlamewakerPriest.DarkMending(eventId, delay, calls, creature)
    ...
    if (which_to_cast == 1) then
        ...
        for index, SulfuronH in pairs(InRange) do
            creature:CastCustomSpell(SulfuronH,
                FlamewakerPriest_Spells.DARK_MENDING, false,
                SulfuronH:GetMaxHealth() / 10)
        end
    elseif which_to_cast == 2 then
        local InRange = creature:GetCreaturesInRange(45, 9000013)
        for index, Priest in pairs(InRange) do
            creature:CastCustomSpell(Priest, FlamewakerPriest_Spells
                .DARK_MENDING, false, Priest:GetMaxHealth() / 10)
        end
    end
end
...
function FlamewakerPriest.ShadowWordPain(eventId, delay, calls, creature)
    ...
    for i = 1, #sample_targets do
        creature:CastCustomSpell(all_targets[sample_targets[i]],
            FlamewakerPriest_Spells.SHADOW_WORD_PAIN, true, damage_calc(110))
    end
end

function FlamewakerPriest.DarkStrike(eventId, delay, calls, creature)
    creature:CastCustomSpell(creature:GetVictim(),
        FlamewakerPriest_Spells.DARK_STRIKE, true, damage_calc(2100))
end
...
function FlamewakerPriest.Immolate(eventId, delay, calls, creature)
    ...
    for i = 1, #sample_targets do
        creature:CastCustomSpell(all_targets[sample_targets[i]],
            FlamewakerPriest_Spells.IMMOLATE, true, damage_calc(900), 1200)
    end
end
